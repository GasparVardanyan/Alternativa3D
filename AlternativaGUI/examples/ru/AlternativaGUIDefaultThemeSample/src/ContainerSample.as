package {
	...
	public class ContainerSample extends GUIobject implements IStageSizeListener {
		...
		public function ContainerSample() {
			...
			// Провайдер данных для выпадающего меню
			...
			// Создаем панель с вкалдками
			tabPanel = new TabPanel();
			textInput = new TextInput();

			// группа для radioButtons
			var radioButtonGroup:RadioButtonGroup = new RadioButtonGroup();

			// группа для toggleButtons
			var toggleButtonGroup:RadioButtonGroup = new RadioButtonGroup();

			// Создаем вертикальный контейнер и указываем зазор между элементами
			vBox = new VBox(5);
			...
			// Контейнер для текстовых компонент
			vBox = new VBox(5);
			vBox.align = Align.LEFT;
			...
			// Создаем DropDownList и наполняем данными
			...
			// Создаем List и наполняем данными
...
//
//
			// Создаем Tree и наполняем данными
			 ...
			 * hasChildren - есть ли у объекта информация дальше
			 * canExpand - папка или нет
			 **/
			for (i = 0; i < 3; i++) {
				obj = new Object();
				obj.parentId = null;
			...
			// Вешаем событие на изменение в радиогруппе
