	...
	public class HingeJointTest extends BasicView
	{
		...
		private function initObject():void
		{
			...
			for (var i:int = 0; i < 8; i++)
			{
				...
				chainBody[i].moveTo(new JNumber3D( 0, 130 + (45 * i + 45), 0));
				if (i > 0) {
			...
			for (i = 1; i < chainBody.length; i++ ){
				...
				chain[i] = new HingeJoint(chainBody[i - 1], chainBody[i], JNumber3D.RIGHT, new JNumber3D(0, 15, 0), 10, 50, 50, 0.1, 0.5);
			}
		}
		...
		private function resetBody():void
		{
			if (chainBody[0].currentState.position.y < -1000)
			{
				...
				{
					chainBody[i].moveTo(new JNumber3D( 0, 800 + (45 * int(i) + 45), 0));
				}
			}
		}
