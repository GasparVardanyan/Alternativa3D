package alternativa.physics.collision.colliders {
	...
	public class BoxSphereCollider implements ICollider {
		...
		public function getContact(prim1:CollisionPrimitive, prim2:CollisionPrimitive, contact:Contact):Boolean {
			...
			box.transform.transformPointTransposed(sPos, center);
			...
			contact.normal.copy(closestPt).transform4(box.transform).subtract(sPos).normalize().reverse();
			contact.pcount = 1;

			var cp:ContactPoint = contact.points[0];
			cp.penetration = sphere.r - Math.sqrt(distSqr);
			cp.pos.copy(contact.normal).scale(-sphere.r).add(sPos);
			cp.r1.diff(cp.pos, sPos);
			cp.r2.diff(cp.pos, bPos);
