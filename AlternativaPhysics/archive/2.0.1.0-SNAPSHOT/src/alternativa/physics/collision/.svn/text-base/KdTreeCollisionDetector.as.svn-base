package alternativa.physics.collision {
	...
	public class KdTreeCollisionDetector implements ICollisionDetector {
		...
		public var dynamicPrimitives:Vector.<CollisionPrimitive>;
		public var dynamicPrimitivesNum:int;
		...
		private var _dynamicIntersection:RayHit = new RayHit();
...
//				}
		 ...
		 */
		public function raycast(origin:Vector3, dir:Vector3, collisionGroup:int, maxTime:Number, predicate:IRayCollisionPredicate, result:RayHit):Boolean {
			var hasStaticIntersection:Boolean = raycastStatic(origin, dir, collisionGroup, maxTime, predicate, result);
		 ...
		 * 
		 */
		public function raycastStatic(origin:Vector3, dir:Vector3, collisionGroup:int, maxTime:Number, predicate:IRayCollisionPredicate, result:RayHit):Boolean {
...
//			return colNum;
		 ...
		 * @return 
		 */
		private function intersectRayWithDynamic(origin:Vector3, dir:Vector3, collisionGroup:int, maxTime:Number, predicate:IRayCollisionPredicate, result:RayHit):Boolean {
			...
			for (var i:int = 0; i < dynamicPrimitivesNum; i++) {
				...
				var t:Number = primitive.raycast(origin, dir, threshold, _n);
				if (t > 0 && t < minTime) {
					minTime = t;
		 ...
		 * @param intersection
		 */
		private function testRayAgainstNode(node:CollisionKdNode, origin:Vector3, localOrigin:Vector3, dir:Vector3, collisionGroup:int, t1:Number, t2:Number, predicate:IRayCollisionPredicate, result:RayHit):Boolean {
		 ...
		 * 
		 */
		private function getRayNodeIntersection(origin:Vector3, dir:Vector3, collisionGroup:int, primitives:Vector.<CollisionPrimitive>, indices:Vector.<int>, predicate:IRayCollisionPredicate, intersection:RayHit):Boolean {
