		...
		private function preProcessContacts(dt:Number):void {
			...
			while (contact != borderContact) {
				...
				for (var j:int = 0; j < contact.pcount; j++) {
					...
					if (b1.movable) {
						cp.angularInertia1 = _v.cross2(cp.r1, contact.normal).transform3(b1.invInertiaWorld).cross(cp.r1).dot(contact.normal);
						cp.velByUnitImpulseN += b1.invMass + cp.angularInertia1;
					}
					if (b2 != null && b2.movable) {
						cp.angularInertia2 = _v.cross2(cp.r2, contact.normal).transform3(b2.invInertiaWorld).cross(cp.r2).dot(contact.normal);
