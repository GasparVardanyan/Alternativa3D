package alternativa.physics.collision.primitives {
	...
	public class CollisionBox extends CollisionPrimitive {
		...
		override public function getRayIntersection(origin:Vector3, vector:Vector3, epsilon:Number, normal:Vector3):Number {
			...
			// Проверка, не находится ли начало луча внутри бокса. В этом случае точкой пересечения будет начало луча,
			// а нормалью будет нормированный вектор, противоположный вектору луча.
//			if (ox >= -hs.x && ox <= hs.x && oy >= -hs.y && oy <= hs.y && oz >= -hs.z && oz <= hs.z) {
//				normal.x = -vector.x;
//				normal.y = -vector.y;
//				normal.z = -vector.z;
//				normal.vNormalize();
